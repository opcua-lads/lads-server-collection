...
//---------------------------------------------------------------
// LADS data-tape extension objects support
//---------------------------------------------------------------
...
export function constructSamplesExtensionObject(addressSpace: IAddressSpace, samples: LADSSampleInfo[]): ExtensionObject[] {
    const ns = getLADSNamespace(addressSpace)
    const dt = addressSpace.findDataType("SampleInfoType", ns.index)
    assert(dt)
    return samples.map(value => constructSampleInfoExtensionObject(dt, value))
}
function constructSampleInfoExtensionObject(dt: UADataType, sampleInfo: LADSSampleInfo): ExtensionObject {
    return dt.addressSpace.constructExtensionObject(dt, {
        ContainerId: sampleInfo.containerId,
        SampleId: sampleInfo.sampleId,
        Position: sampleInfo.position,
        CustomData: sampleInfo.customData,
    })
}

export function constructPropertiesExtensionObject(addressSpace: IAddressSpace, properties: LADSProperty[]): ExtensionObject[] {
    const ns = getLADSNamespace(addressSpace)
    const dt = addressSpace.findDataType("KeyValueType", ns.index)
    assert(dt)
    return properties.map(value => constructPropertyExtensionObject(dt, value))
}
function constructPropertyExtensionObject(dt: UADataType, property: LADSProperty): ExtensionObject {
    return dt.addressSpace.constructExtensionObject(dt, {
        Key: property.key,
