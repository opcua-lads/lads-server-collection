    ...
    constructor(server: AtmoWebServerImpl, config: AtmoWebDeviceConfig) {
        ...
        this.unitImpl.on("initialized", (data: any) => {
            ...
            function imageFile(model: string): string {
                const defaultImage = "memmert-unplus.png"
                if (!model) return defaultImage
                if (model.includes("UN"))
                    return defaultImage
                else if (model.includes("IN"))
                    return "memmert-inplus.png"
                else if (model.includes("VO"))
                    return "memmert-vo.png"
                else if (model.includes("ICO"))
                    return "memmert-ico.png"
                return defaultImage
            }
            const imagesDir = join(__dirname, "resources", "images")
            DeviceTypeImage.create({parent: this.device, imagesDir, imageFiles: [imageFile(deviceOptions.model)] })
